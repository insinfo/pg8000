...
import 'code_points/non_ascii_spaces.dart';
...
class Saslprep {
  ...
  static String saslprep(String input, {SaslprepOptions? options}) {
    ...
    // 1. Map
    final mappedInput = toCodePoints(input)
        // 1.1 mapping to space
        .map((character) =>
            (nonAsciiSpaceCharacters.contains(character) ? 0x20 : character))
        // 1.2 mapping to nothing
        .where((character) => !commonlyMappedToNothing.contains(character));

    // 2. Normalize
    final normalizedInput = unorm.nfkc(String.fromCharCodes(mappedInput));

    final normalizedMap = toCodePoints(normalizedInput);

    // 3. Prohibit
    final hasProhibited =
        normalizedMap.any((character) => prohibitedCharacters.contains(character));
    ...
    if (options == null || options.allowUnassigned != true) {
      final hasUnassigned =
          normalizedMap.any((character) => unassignedCodePoints.contains(character));
      if (hasUnassigned) {
        throw Exception(
    ...
    // 4. check bidi
    final hasBidiRAL =
        normalizedMap.any((character) => bidirectionalRAl.contains(character));
    final hasBidiL =
        normalizedMap.any((character) => bidirectionalL.contains(character));
    ...
    final isFirstBidiRAL =
        bidirectionalRAl.contains(normalizedInput.codeUnitAt(0));
    final isLastBidiRAL = bidirectionalRAl
        .contains(normalizedInput.codeUnitAt(normalizedInput.length - 1));
