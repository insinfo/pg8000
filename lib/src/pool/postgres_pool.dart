...
import 'package:dargres/src/dependencies/executor/executor.dart';
import 'package:dargres/src/dependencies/retry/retry.dart';
...
typedef PgSessionFn<R> = Future<R> Function(ExecutionContext c);
...
class PgPool implements ExecutionContext {
      ...
      return await retryOptions.retry(
        () async {
          return await _withConnection((c) async {
            return await c.connection.runInTransaction(
  ...
  Future<bool> _testConnection(_ConnectionCtx ctx) async {
    ...
    try {
      await ctx.connection.querySimple('SELECT 1;');
      return true;
    } catch (_) {}
    return false;
  }
  ...
  int get queueSize =>
      _connections.fold<int>(0, (a, b) => a + b.connection.queryQueueSize);
  ...
  Future<Results> queryUnnamed(
    ...
    return run(
        (c) => c.queryUnnamed(sql, params,
            placeholderIdentifier: placeholderIdentifier, timeout: timeout),
        sessionId: sessionId,
        traceId: traceId);
  }
  ...
  Future<Results> querySimple(
    ...
    return run(
      (c) => c.querySimple(sql, timeout: timeout),
      sessionId: sessionId,
      traceId: traceId,
    );
  }
  ...
  Future<void> _setApplicationName(
    ...
    await pgConn.execute(
      'SET application_name = $effectiveName',
      // substitutionValues: {
...
class _PgExecutionContextWrapper implements ExecutionContext {
  final int connectionId;
  final ExecutionContext _delegate;
  final String? sessionId;
  final String? traceId;
  ...
  Future<Results> queryUnnamed(String sql, dynamic params,
      ...
      Duration? timeout = const Duration(seconds: 60)}) {
    return _run(
      () => _delegate.queryUnnamed(sql, params,
          placeholderIdentifier: placeholderIdentifier,
  ...
  Future<Results> queryNamed(String sql, dynamic params,
      ...
      Duration? timeout = const Duration(seconds: 60)}) {
    return _run(
      () => _delegate.queryNamed(sql, params,
          placeholderIdentifier: placeholderIdentifier,
          isDeallocate: isDeallocate,
          timeout: timeout),
      sql,
  ...
  Future<Results> querySimple(String sql,
      {Duration? timeout = const Duration(seconds: 60)}) {
    return _run(
      () => _delegate.querySimple(sql, timeout: timeout),
      sql,
      [],
    );
  }
  ...
  @override
  Future<Results> executeStatement(Query query, {bool isDeallocate = false, Duration? timeout = const Duration(seconds: 60)}) {
    throw UnimplementedError();
  }
  ...
  @override
  Future<Query> prepareStatement(String sql, params,
      {bool isUnamedStatement = false,
      PlaceholderIdentifier placeholderIdentifier =
          PlaceholderIdentifier.pgDefault, Duration? timeout = const Duration(seconds: 60)}) {
    throw UnimplementedError();
  }
