...
void main() async {
  group('Scram Authentication', () {
    ...
    for (final scramExample in scramExamples) {
      ...
      test('Should be able to return the client first message for the server',
        ...
        expect(parsed['n'], '*');
        expect(parsed['r'], scramExample.clientNonce());
      ...
      test(
        ...
        final bytesReceivedFromServer =
            coerceUint8List(utf8.encode(scramExample.serverFirstMessage()));
        ...
        expect(parsed['c'], scramExample.gs2HeaderBase64());
        expect(parsed['r'], scramExample.fullNonce());
        expect(parsed['p'], scramExample.clientFinalMessageProof());
      });
      ...
      test(
          ...
          () {
        final bytesReceivedFromServer =
            coerceUint8List(utf8.encode(scramExample.serverFinalMessage()));
        ...
        expect(serverSignature, scramExample.serverFinalMessageProof());
  ...
  group('ClientLast signature validation', () {
    test('rejects a signature with different bytes', () {
      final step = ClientLast(Uint8List(0), Uint8List.fromList(<int>[1, 2, 3]));
      final response = utf8.encode('v=${base64.encode(<int>[1, 2, 4])}');

      expect(() => step.transition(response), throwsA(isA<SaslScramException>()));
    });

    test('rejects a signature with different length', () {
      final step = ClientLast(Uint8List(0), Uint8List.fromList(<int>[1, 2, 3]));
      final response = utf8.encode('v=${base64.encode(<int>[1, 2])}');
